---
title: Phase 1, session 3′ — the early probes
status: IN PROGRESS. Four design-gating unknowns, measured before the clip block
        is designed. ⚠⚠ **E20a ●● PASS live 2026-08-09, 12/12** — `launchWithOptions`
        confirms E18-VERDICT §4a″-bis by measurement (`FINDINGS.md` E20a), after
        two APPARATUS bugs that first produced a confident refutation of a claim
        that is true (see §What the first live sitting cost).
        ⚠⚠ **E20b ●● PASS live 11/11** — `duplicateClip` OVERWRITES an occupied
        next row and the launcher-content observer cannot see it, so an empty
        destination is a HARD PRECONDITION for 3″ (`FINDINGS.md` E20b).
        ● E20c ARM A green (7/7, no DAW). ⚠ E20d not yet run; three arms are
        operator-gated (the ear check, the annotation prompts, the doc-state
        restart).
updated: 2026-08-09
...
# Phase 1, session 3′ — the early probes
...
`PHASE-1-ENGINE.md`'s re-plan puts a row 3′ between session 3 and the branch
...
|---|---|---|---|
| 1 | `launchWithOptions(quantization, launchMode)` | per-call `"1"`/`"8"` quantisation, and ⚠⚠ `"continue_or_synced"` — take B resumes at A's position, **the only answer to E16m** and the one thing no mute, solo or chain switch can imitate (§4c) | ⚠⚠ ●● **MEASURED 2026-08-09, 12/12** (`FINDINGS.md` E20a). Both claims hold; ⚠ ear arm owed |
...
| 4 | `getDocumentState()` JSON capacity | where **D18d's branch-event record** lives | ⚠⚠ **ANSWERED 2026-08-09, and the question was the wrong one** (`FINDINGS.md` E20d). All three storage ceilings ● at **262144 chars** — wire, setting, and a full save+restart. ⚠⚠ But **interacting with the field HARD-LOCKED BITWIG** (force quit). The constraint is that the value is DRAWN, not that it is large. ⚠ `probe:e20d-hidden` tests the operator's fix |
...
- **The clip block's design, the tool surface, D18c's fresh naming** — session 3″.
...
- **Standing rule 5 for scenes** (session 3's B1/B2 → the proposed 3‴). `E20b`
  refuses rather than fixes: it checks the budget, and it will not delete a clip
  to make room.
- **The stash / reversal wiring** (session 3's B3 → 3″).
...
2. ◐ **HALF MET** — `"continue_or_synced"` has ⚠⚠ **a number**: take B enters where
   take A was, separated from the transport grid by 31 steps and from B's own last
   position by 17. ⚠ The **ear** is owed (`probe:e20a-ear`).
3. ● **MET** — `duplicateClip` lands in the next row down through **both** routes,
   ...
   precondition 3″ must enforce.
4. Claude Code's real behaviour on `destructiveHint` recorded **per tool, against
   the unannotated baseline**, and D20 either confirmed or given a proposed
   amendment.
5. `getDocumentState()` capacity reported as **three separate numbers** plus a
   survives-a-restart verdict, and the *drawable* ceiling noted beside the
   storable one.
6. `probe:hello` green against the redeployed extension with the deploy check
   reporting `fresh`; `npm run check` green.
...
- **`duplicateClip` inserting rather than appending** would not kill the clip
  ...
  (D6). Worth knowing before 3″ builds on the other assumption.
