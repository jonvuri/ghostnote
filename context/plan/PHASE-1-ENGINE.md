...
status: in progress — split into six sessions 2026-07-25 (see §Session index),
        ...
        3c (the window) DONE 2026-08-09; session 3d (the write surface) DONE
        2026-08-10 — the engine has a caller at last, and session 3's B3 is closed.
...
| 3d | ⚠ **NEW — the write surface** | ● **DONE 2026-08-10** — see [SESSION-3D](PHASE-1-SESSION-3D-SURFACE.md). **18 tools** in `brain/src/surface/` over the existing `Op` union (no branch verbs), partitioned read / write / destructive by **NAME** per D20 as E20c measured it, with four separate delete names because *"don't ask again for this tool"* is a per-name blanket grant. **B3 wired**: `Workspace.apply` is `executor.run` → `stash.record` in one expression and no tool can see an executor to go around it; every reversal is planned against the launcher window, and ⚠ **`moved` is produced through the production path** — a clip replaced by an identical one is refused, with a control proving the window is what catches it. D18c naming from the first line, held by a lexical ban list (`naming.ts`) asserted against the JSON schema an agent receives, the whole refusal catalogue and everything the suite emitted; ⚠ nothing internal is forwarded verbatim, so the surface re-says verdicts, losses and refusals from DATA. Offline **344/344**. ⚠⚠ **Two things it found**: the fidelity floor refuses **every** directed deletion (D18c vs D20 — closed with a third `Clearance` kind, PROPOSED below), and the contract could not **enumerate** anything, so it gained its eighth method `tracks()` with a conformance case. ⚠ The live arm is **narrow and is not a tool-set sweep** (that is session 5's): `probe:e09` plus the new `C-list` case are the only live paths through anything this session wrote. ● **Live conformance 45/0/6** on 2026-08-10 — `C-list` green, so `LiveAdapter.tracks()` is proven live. ⚠⚠ `C-minted` passed in that run and is **not** retired by it: 3c diagnosed a timing flake that fails closed, and a green run is what that predicts some of the time — session 5's B7 stands. ⚠ `probe:e09` FAILED its check C on first run, on a hard-coded fixture assumption that predates the re-point (`gn-A` row 0 holds whatever the last probe left there); replaced by a comparison against a direct adapter read, ● **green at 4/4**. ⚠ v1 is **not frozen here**: a description cohort needs branch events, and there are none yet |
...
### ⚠ Decisions PROPOSED by session 3d, awaiting the operator (rule 10)

Built and tested, recorded here rather than in `DECISIONS.md`. The first one
amends how two settled decisions meet; the other two are shape.

1. ⚠⚠ **A third clearance: `directed-destruction`.** D18c's fidelity floor refuses
   any batch whose prior state cannot be reproduced exactly — which is **every**
   deletion of anything that already exists, since a track's identity, a row's
   arrangement and a device's settings have no readback at all. D20 says the
   opposite about exactly these calls: *"the boundary is host-mediated: nothing
   INSIDE our system gates a directed destructive call"*, and the agent *"may
   EXECUTE destruction the operator explicitly directed."* Unresolved, D20's
   destructive surface could never run once. ⇒ The four destructive tools carry a
   clearance stamped with the tool **NAME** the call rode in on, because the name
   is what the operator granted (E20c). ⚠ *Rejected: leaving the floor to refuse
   them.* That is not conservatism, it is shipping a partition that cannot
   execute — and it would push the pressure onto the ordinary write surface, which
   is where a destructive verb must never end up. ⚠ It clears the REFUSAL and
   nothing else: `gateBeforeReading` still refuses (being unable to say what a
   deletion destroys is the one wall no permission moves, D20), and every
   destructive receipt still reports what it could not put back.
2. **The contract's eighth method, `tracks()`.** `resolve` maps identity to a
   handle and `read` reads addresses a caller already holds; nothing could answer
   *which addresses exist*, and no `Op` or `Address` variant could — an
   enumeration has nothing to be addressed by. ⚠ *Rejected: a `Session`-level
   listing over the wire method the live adapter already calls.* It would have kept
   the method count at seven and made the one thing every agent starts with the one
   thing the conformance suite could not check on both adapters.
3. **The write surface is recorded by CONSTRUCTION, not by convention.** A tool
   receives a `Workspace`, whose `apply` is `executor.run` → `stash.record` in one
   expression, and whose `changes` is the stash's read half. ⚠ *Rejected: tools
   calling the executor and recording afterwards.* It satisfies B3 on the day it is
   written and fails the first time somebody adds a tool in a hurry.
