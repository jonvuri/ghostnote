...
| I02 | `BitwigAdapter`, `Address`, `Op`, `AdapterInfo`; `ghostnote/0`; [contract](../../../brain/src/contract/adapter.ts), [live capture adapter](../../../brain/src/audio/live-audio-capture.ts) | Contract/adapter; engine ↔ fake or live adapter; addressed operations and capability checks | Retain exact-equality contract. The 7e capture controller is a separate typed live-adapter capability. It keeps `ghostnote/0` and requires the exact method hash and deployment check. |
...
| I15 | `ghostnote-saved-bitwig-project-v0`, `ghostnote-audio-capture-source-v0`, `ghostnote-audio-capture-request-v0`, `audio-capture-v0`; [capture module](../../../brain/src/audio/audio-capture.ts) | Capture module; saved-project and live guards → recorder/directory lifecycle → exact artifact declaration | Retain as an experimental 7e boundary. Capture has no embedded signal facts. Its full file SHA-256, format, scope, permission, and creator form the separate I16 input declaration. |
...
| I24 | Module descriptor `ghostnote-workstation-module-v0`, correlated request/result, experimental run record | Module coordinator; configured modules ↔ host/run log; discovery, correlation, and run provenance | The 7a registry implements lazy discovery, deadlines and correlation. The 7c documentation module adds source-set correlation. The 7d audio module adds separate executable discovery. The 7e capture module adds pre-start saved-project checks, fresh live discovery, exact source correlation, and independent capture/analysis results. The composed run record remains 7f work. |
...
|---|---|---|---|
| `phase6a-master-recorder.ts` | Extracted in 7e: directory attribution, recorder lifecycle, artifact identity, and narrow header validation | Three-capture identity and cleanup control plus E125 product fixtures | Product design has no hard-coded Homebrew path or embedded signal analysis. 8b retains required typed recorder methods and classifies remaining probe allocation. |
