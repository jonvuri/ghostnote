...
| I04 | `NoteRecord`, `StateValue`, `Snapshot`, `RevisionMark`, `BatchReceipt`; `ghostnote/0`; [state](../../../brain/src/contract/state.ts), [snapshot](../../../brain/src/contract/snapshot.ts) | Adapter; host/fake reads → engine/stash/compiler; observed state and guards | Retain complete host state. Revise with `ghostnote-exact-note-source-v0` wrapper for symbolic consumers. Do not change Snapshot semantics. |
...
| I14 | `ghostnote-sensory-packet-v0`, routed A/B fields and deltas; [packet probe](../../../brain/src/probes/phase6h-sensory-packets.py) | Task router; typed MIDI/audio facts → host agent; decision evidence only | Revise to planned v1 projection. Remove duplicate note state and unused edit fields; keep field-level provenance and comparison limits. |
...
| I16 | E105 manifest schema 1, typed audio result dicts, E118 FFmpeg fact records, WAV/diagnostic files; [audio probe](../../../brain/src/probes/phase6c-audio-analysis.py) | Audio providers; verified bytes → facts/estimates or diagnostic image; measured signal evidence | Merge metadata into `audio-facts-v0`; retain E118's selected formulas. WAV is standard; its provenance manifest is custom. Images remain diagnostics. |
| I17 | Official document requests/cache manifest `schemaVersion: 1`; [cache library](../../../brain/src/probes/phase6b-document-cache-lib.ts) | Documentation cache; installed/downloaded bytes → verified cache reader; source identity/compatibility | Retain v1 downloaded manifest semantics. Revise installed-tree source wrapper for common provenance. No automatic schema migration. |
| I18 | E104 corpus dicts (`id/source/title/content`), SQLite FTS5 index, ranked retrieval JSON; unversioned; [retrieval probe](../../../brain/src/probes/phase6b-retrieval.py) | Documentation retrieval; verified sources → bounded cited matches; evidence selection | Revise to `documentation-v0` result. Index is disposable and tied to source hashes, tokenizer and extractor versions. Retire LSA from the product route. |
...
| I24 | Planned module descriptor `ghostnote-workstation-module-v0`, worker request/result, experimental run record | Module coordinator; configured modules ↔ host/run log; discovery, correlation, and run provenance | Revise checkout-only orchestration into explicit experimental records in 7a/7f. No global provider runtime or hidden shared state. |
...
| `phase6b-document-cache-lib.ts`, `phase6b-document-cache.ts`, `phase6b-retrieval.py` | Verified cache and source-routed FTS5 in 7c | Cache corruption, offline refusal and nine routed questions | LSA/NumPy comparison and inventory runner remain evidence only. |
| `phase6c-audio-analysis.py` | Selected executable fact adapters; isolated worker only when needed in 7d | Known-signal/tolerance controls, including silence | Essentia/aubio comparison and broad unused metrics remain evidence only. |
...
| `phase6h-sensory-packets.py` | Selected formulas and task routing in 7a/7d | Same-source, silence, undefined-property, paired-delta and level controls | Raw-inventory arms, model responses and ballot machinery remain evidence only. |
