...
| I24 | Module descriptor `ghostnote-workstation-module-v0`, correlated request/result, experimental run record | Module coordinator; configured modules ↔ host/run log; discovery, correlation, and run provenance | The 7a in-process registry implements lazy discovery, deadlines and correlation. The 7c documentation module adds source-set correlation and family-local dependency startup. Later worker modules add only their private framed protocols. The composed run record remains 7f work. |
