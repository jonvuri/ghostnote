...
| S17 / I24 and all new modules | Configured descriptor → isolated startup → correlated request/result | In-process registry implemented in 7a: [registry tests](../../../brain/src/workstation/module-registry.test.ts) cover inert discovery, isolated missing Python, unrelated availability, late and mismatched replies, and no retry. The 7c documentation module adds source-digest correlation, bounded startup/request deadlines and family-local PDF startup. Later executable or worker modules still own their dependency, framing and cache fixtures. |
